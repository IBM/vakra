#!/usr/bin/env python3
"""
Benchmark Runner (One Container Per Task)
==========================================

Each task runs against its own dedicated container for memory isolation
and crash protection. All containers use the same m3_environ Docker image.

Containers:
  task_2_m3_environ  — M3 REST only (no retriever)
  task_5_m3_environ  — M3 REST + Retriever

Setup:
  # Start containers (or use --start-containers flag)
...
# ---------------------------------------------------------------------------
# Task configurations — each task has its own container
# ---------------------------------------------------------------------------
@dataclass
class TaskConfig:
    """Per-task settings including the dedicated container name."""
    input_dir: str
    container_name: str
...
TASK_CONFIGS: Dict[int, TaskConfig] = {
    ...
    5: TaskConfig(
        ...
        volume_mounts=[
            "apis/m3/rest/db:/app/db:ro",
            "apis/configs:/app/apis/configs:ro",
            "apis/retrievers/chroma_data:/app/retrievers/chroma_data",
            "apis/retrievers/queries:/app/retrievers/queries:ro",
        ],
    ),
}
...
def _build_exec_args(
    ...
    """Build docker/podman exec args for starting an MCP server in a container."""
    args = [
        "exec", "-i",
...
def is_container_running(container_runtime: str, container_name: str) -> bool:
...
def start_container(
    ...
    """Start a container if not already running. Waits for readiness."""
    if is_container_running(container_runtime, container_name):
    ...
    cwd = os.getcwd()
    cmd = [container_runtime, "run", "-d", "--name", container_name]
    for mount in volume_mounts:
        host_path, rest = mount.split(":", 1)
        abs_host = os.path.join(cwd, host_path) if not os.path.isabs(host_path) else host_path
        cmd.extend(["-v", f"{abs_host}:{rest}"])
    cmd.append(image)
    ...
    # Wait for readiness
    print(f"  Waiting for '{container_name}' to be ready (up to {timeout_s}s)...")
    for i in range(1, timeout_s + 1):
        log_result = subprocess.run(
            [container_runtime, "logs", container_name],
            capture_output=True, text=True,
        )
        if "All services running" in log_result.stdout or "All services running" in log_result.stderr:
            print(f"  Container '{container_name}' is ready. ({i}s)")
            return
        import time
        time.sleep(1)

    print(f"  ERROR: Container '{container_name}' did not become ready within {timeout_s}s.")
    print(f"  Logs: {subprocess.run([container_runtime, 'logs', '--tail', '20', container_name], capture_output=True, text=True).stdout}")
    sys.exit(1)


def stop_container(container_runtime: str, container_name: str):
    """Stop and remove a container."""
    print(f"  Stopping container '{container_name}'...")
    subprocess.run([container_runtime, "stop", container_name], capture_output=True, timeout=30)
    subprocess.run([container_runtime, "rm", container_name], capture_output=True, timeout=10)
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark Runner (One Container Per Task)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Each task runs in its own container for memory isolation and crash protection.
All containers use the same m3_environ Docker image.

  task_2_m3_environ  — M3 REST only
  task_5_m3_environ  — M3 REST + Retriever
...
Container management:
  ...
  python benchmark_runner_one_per_task.py --task_id 2 5 --run-agent --domain address --cleanup
    ...
    parser.add_argument("--start-containers", action="store_true", help="Auto-start required containers before running")
    parser.add_argument("--cleanup", action="store_true", help="Stop and remove containers after benchmark completes")
    parser.add_argument("--docker-image", type=str, default=DOCKER_IMAGE, help=f"Docker image to use (default: {DOCKER_IMAGE})")
    ...
    if args.start_containers:
        ...
        for tid in task_ids:
            cfg = task_cfgs[tid]
            start_container(
                container_runtime, cfg.container_name, cfg.volume_mounts,
                image=args.docker_image,
            )
        print("--- All containers ready ---\n")

    mode = "parallel" if args.parallel and len(task_ids) > 1 else "sequential"
    containers = [task_cfgs[tid].container_name for tid in task_ids]
    print("=" * 60)
    print(f"Benchmark Runner — One Per Task ({mode}, tasks: {task_ids})")
    ...
    try:
        ...
        if args.cleanup:
            ...
            for tid in task_ids:
                cfg = task_cfgs[tid]
                stop_container(container_runtime, cfg.container_name)
