...
async def connect_to_server(
    ...
    container_name: str = "m3-sel-slot-server",
    ...
    Args:
        ...
        container_name: Container name for docker mode (default: m3-sel-slot-server)
    ...
    if mode == "stdio":
        ...
        # Docker/Podman exec: run MCP server inside a running container
...
async def run_single_server_with_instances(
    ...
    Args:
        ...
        container_name: Container name (docker mode only, default: m3-sel-slot-server)
...
async def main(
    ...
    container_name="m3-sel-slot-server",
    domain=None,
):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="MCP Demo — run LLM agents against the M3 python_tools MCP server.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Connection modes:
  stdio      Launch MCP server as a local subprocess (default)
  docker     Exec into a running Docker/Podman container
...
Examples:
  ...
  # WebSocket
  python examples/demo.py --mode websocket --server-url ws://localhost:8000/mcp
        """,
    )
    ...
    docker_group.add_argument(
        "--container-name",
        type=str,
        default="m3-sel-slot-server",
        help="Name of the running container (default: m3-sel-slot-server)"
    )
    docker_group.add_argument(
        "--domain",
        type=str,
